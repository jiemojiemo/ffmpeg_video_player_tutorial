...
TEST_F(AFFMPEGAudioResampler, PrepareWillInitChannelData) {
    ...
    size_t expected = FormatUtils::getChannelDataSize(out_num_channels, out_sample_format);
...
TEST_F(AFFMPEGAudioResampler, CanConvertAudioSamples) {
    ...
    std::vector<const uint8_t *> data_refer_to{(uint8_t *)left_data.data(), (uint8_t *)right_data.data()};
