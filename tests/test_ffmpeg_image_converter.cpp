...
public:
    ...
    void allocateInFrame() {
        ...
        av_image_fill_arrays(in_frame->data,
                             in_frame->linesize,
                             in_frame_buffer,
                             src_format,
                             src_w,
...
TEST_F(AFFMPEGImageConverter, PrepareWillAllocateInternalBufferAndFrame) {
    c.prepare(src_w,
              src_h,
              src_format,
              dst_w,
    ...
    ASSERT_THAT(c.frame, NotNull());
    ASSERT_THAT(c.frame_buffer, NotNull());
}
