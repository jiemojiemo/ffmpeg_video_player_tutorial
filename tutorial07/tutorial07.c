 ...
 *
 *   File:   tutorial07.c
 *           This tutorial adds seeking functionalities to the player coded in
 *tutorial06.c
 *
 *           Compiled using
 *               gcc -o tutorial07 tutorial07.c -lavutil -lavformat -lavcodec
 *-lswscale -lswresample -lz -lm  `sdl2-config --cflags --libs` on Arch Linux.
 *
...
/**
 * Struct used to hold the format context, the indices of the audio and video
 * stream, the corresponding AVStream objects, the audio and video codec
 * information, the audio and video queues and buffers, the global quit flag and
...
static int64_t guess_correct_pts(AVCodecContext *ctx, int64_t reordered_pts,
                                 int64_t dts);

double synchronize_video(VideoState *videoState, AVFrame *src_frame,
                         double pts);
...
int main(int argc, char *argv[]) {
  ...
  // the global VideoState reference will be set in decode_thread() to this
  // pointer
  ...
  videoState->decode_tid =
      SDL_CreateThread(decode_thread, "Decoding Thread", videoState);
 ...
 * an infinite loop to read AVPackets from the global VideoState
 * AVFormatContext. Based on their stream index, each packet is placed in the
 * appropriate queue.
 *
...
int decode_thread(void *arg) {
  ...
  // file I/O context: demuxers read a media file and split it into chunks of
  // data (packets)
  ...
  for (;;) {
    ...
    if (videoState->seek_req) {
      ...
      if (video_stream_index >= 0 && audio_stream_index >= 0) {
        seek_target_video =
            av_rescale_q(seek_target_video, AV_TIME_BASE_Q,
                         pFormatCtx->streams[video_stream_index]->time_base);
        seek_target_audio =
            av_rescale_q(seek_target_audio, AV_TIME_BASE_Q,
                         pFormatCtx->streams[audio_stream_index]->time_base);
      }

      ret = av_seek_frame(videoState->pFormatCtx, video_stream_index,
                          seek_target_video, videoState->seek_flags);
      ret &= av_seek_frame(videoState->pFormatCtx, audio_stream_index,
                           seek_target_audio, videoState->seek_flags);
...
/**
 * Retrieves the AVCodec and initializes the AVCodecContext for the given
 * AVStream index. In case of AVMEDIA_TYPE_AUDIO codec type, it sets the desired
...
int stream_component_open(VideoState *videoState, int stream_index) {
  ...
  case AVMEDIA_TYPE_VIDEO: {
    ...
    // start video thread
    videoState->video_tid =
        SDL_CreateThread(video_thread, "Video Thread", videoState);
    ...
    videoState->sws_ctx = sws_getContext(
        ...
        videoState->video_ctx->height, AV_PIX_FMT_YUV420P, SWS_BILINEAR, NULL,
        NULL, NULL);
...
void alloc_picture(void *userdata) {
  ...
  int numBytes;
  numBytes =
      av_image_get_buffer_size(AV_PIX_FMT_YUV420P, videoState->video_ctx->width,
                               videoState->video_ctx->height, 32);
  ...
  // The fields of the given image are filled in by using the buffer which
  // points to the image data buffer.
  av_image_fill_arrays(videoPicture->frame->data, videoPicture->frame->linesize,
                       buffer, AV_PIX_FMT_YUV420P, videoState->video_ctx->width,
                       videoState->video_ctx->height, 32);
...
/**
 * Waits for space in the VideoPicture queue. Allocates a new SDL_Overlay in
 * case it is not already allocated or has a different width/height. Converts
 * the given decoded AVFrame to an AVPicture using specs supported by SDL and
 * writes it in the VideoPicture queue.
 *
 * @param   videoState  the global VideoState reference.
 * @param   pFrame      AVFrame to be inserted in the VideoState->pictq (as an
 * AVPicture).
 *
...
int queue_picture(VideoState *videoState, AVFrame *pFrame, double pts) {
  ...
  // if the VideoPicture SDL_Overlay is not allocated or has a different
  // width/height
  if (!videoPicture->frame ||
      videoPicture->width != videoState->video_ctx->width ||
      videoPicture->height != videoState->video_ctx->height) {
  ...
  if (videoPicture->frame) {
    ...
    // scale the image in pFrame->data and put the resulting scaled image in
    // pict->data
    sws_scale(videoState->sws_ctx, (uint8_t const *const *)pFrame->data,
              pFrame->linesize, 0, videoState->video_ctx->height,
              videoPicture->frame->data, videoPicture->frame->linesize);
 ...
 * This function is used as callback for the SDL_Thread.
 *
 * This thread reads in packets from the video queue, packet_queue_get(),
 * decodes the video packets into a frame, and then calls the queue_picture()
 * function to put the processed frame into the picture queue.
 *
 * @param   arg the data pointer passed to the SDL_Thread callback function.
 *
 * @return
...
int video_thread(void *arg) {
  ...
  for (;;) {
    ...
    while (ret >= 0) {
      ...
      pts = guess_correct_pts(videoState->video_ctx, pFrame->pts,
                              pFrame->pkt_dts);
...
/**
 * Updates the PTS of the last decoded video frame to be in sync with
 * everything. This function will also deal with cases where we don't get a PTS
 * value for our frame. At the same time we need to keep track of when the next
 * frame is expected so we can set our refresh rate properly. We can accomplish
 * this by using the VideoState internal video_clock value which keeps track of
 ...
 *
 * @return              the updated (synchronized) pts for the given video
 * AVFrame.
 */
double synchronize_video(VideoState *videoState, AVFrame *src_frame,
                         double pts) {
 ...
 * we've gotten N audio sample sets that have been out of sync. The amount we
 * are out of sync can also vary a good deal, so we're going to take an average
 * of how far each of those have been out of sync. So for example, the first
 * call might have shown we were out of sync by 40ms, the next by 50ms, and so
 * on. But we're not going to take a simple average because the most recent
 * values are more important than the previous ones. So we're going to use a
 * fractional coefficient, say c, and sum the differences like this: diff_sum =
 * new_diff + diff_sum*c. When we are ready to find the average difference, we
 * simply calculate avg_diff = diff_sum * (1-c).
 *
 * @param   videoState      the global VideoState reference.
 ...
 * @return
 */
int synchronize_audio(VideoState *videoState, short *samples,
                      int samples_size) {
  int n;
  double ref_clock;
  ...
  if (videoState->av_sync_type != AV_SYNC_AUDIO_MASTER) {
    ...
    if (diff < AV_NOSYNC_THRESHOLD) {
      ...
      videoState->audio_diff_cum =
          diff + videoState->audio_diff_avg_coef * videoState->audio_diff_cum;

      if (videoState->audio_diff_avg_count < AUDIO_DIFF_AVG_NB) {
        videoState->audio_diff_avg_count++;
      } else {
        avg_diff = videoState->audio_diff_cum *
                   (1.0 - videoState->audio_diff_avg_coef);
        ...
        if (fabs(avg_diff) >= videoState->audio_diff_threshold) {
          wanted_size = samples_size +
                        ((int)(diff * videoState->audio_ctx->sample_rate) * n);
          min_size =
          ...
          /**
           * Now we have to actually correct the audio. You may have noticed
           * that our synchronize_audio function returns a sample size, which
           * will then tell us how many bytes to send to the stream. So we just
           * have to adjust the sample size to the wanted_size. This works for
           * making the sample size smaller. But if we want to make it bigger,
           * we can't just make the sample size larger because there's no more
           * data in the buffer! So we have to add it. But what should we add?
           * It would be foolish to try and extrapolate audio, so let's just use
           * the audio we already have by padding out the buffer with the value
           * of the last sample.
           */
 ...
 * when the next video frame should be shown, calls the video_display() method
 * to actually show the video on the screen, then decrements the counter on the
 * queue, and decreases its size.
 *
...
double get_video_clock(VideoState *videoState) {
  double delta =
      (av_gettime() - videoState->video_current_pts_time) / 1000000.0;

  return videoState->video_current_pts + delta;
}

/**
 * Calculates and returns the current external clock reference value: the
 * computer clock.
 *
 ...
 * @param   videoState  the global VideoState reference.
 *
 * @return              the reference clock according to the chosen AV sync
 * type.
 */
double get_master_clock(VideoState *videoState) {
  if (videoState->av_sync_type == AV_SYNC_VIDEO_MASTER) {
 ...
 * timer, which will trigger an event, which will have our main() function in
 * turn call a function that pulls a frame from our picture queue and displays
 * it.
 *
 * @param   videoState  the global VideoState reference.
...
/**
 * Retrieves the video aspect ratio first, which is just the width divided by
 * the height. Then it scales the movie to fit as big as possible in our screen
...
void video_display(VideoState *videoState) {
  ...
  if (!screen) {
    ...
    screen = SDL_CreateWindow("FFmpeg SDL Video Player",
  ...
  if (!videoState->texture) {
    ...
    videoState->texture = SDL_CreateTexture(
        videoState->renderer, SDL_PIXELFORMAT_YV12, SDL_TEXTUREACCESS_STREAMING,
        videoState->video_ctx->width, videoState->video_ctx->height);
  }
  ...
  if (videoPicture->frame) {
    ...
    if (w > screen_width) {
      ...
      // recalculate height using the calculated aspect ratio and the screen
      // width
    ...
    if (++videoState->currentFrameIndex < videoState->maxFramesToDecode) {
      if (_DEBUG_) {
        ...
        printf("Frame %c (%d) pts %" PRId64 " dts %" PRId64
               " key_frame %d [coded_picture_number %d, display_picture_number "
               "%d, %dx%d]\n",
               av_get_picture_type_char(videoPicture->frame->pict_type),
               videoState->video_ctx->frame_number, videoPicture->frame->pts,
               videoPicture->frame->pkt_dts, videoPicture->frame->key_frame,
               videoPicture->frame->coded_picture_number,
               videoPicture->frame->display_picture_number,
               videoPicture->frame->width, videoPicture->frame->height);
      }
      ...
      SDL_UpdateYUVTexture(
          videoState->texture, NULL, videoPicture->frame->data[0],
          videoPicture->frame->linesize[0], videoPicture->frame->data[1],
          videoPicture->frame->linesize[1], videoPicture->frame->data[2],
 ...
 *
 * @return          0 if the AVPacket is correctly inserted in the given
 * PacketQueue.
 */
int packet_queue_put(PacketQueue *queue, AVPacket *packet) {
 ...
 * @param   blocking    0 to avoid waiting for an AVPacket to be inserted in the
 * given queue, != 0 otherwise.
 *
 * @return              < 0 if returning because the quit flag is set, 0 if the
 * queue is empty, 1 if it is not empty and a packet was extracted.
 */
static int packet_queue_get(PacketQueue *queue, AVPacket *packet,
  ...
  for (;;) {
    ...
    if (avPacketList) {
      ...
      // point packet to the extracted packet, this will return to the calling
      // function
...
void audio_callback(void *userdata, Uint8 *stream, int len) {
  ...
  while (len > 0) {
    ...
    if (videoState->audio_buf_index >= videoState->audio_buf_size) {
      ...
      int audio_size = audio_decode_frame(videoState, videoState->audio_buf,
                                          sizeof(videoState->audio_buf), &pts);
      ...
      if (audio_size < 0) {
        ...
        audio_size = synchronize_audio(
            videoState, (int16_t *)videoState->audio_buf, audio_size);
    ...
    memcpy(stream,
           (uint8_t *)videoState->audio_buf + videoState->audio_buf_index,
           len1);
  ...
  for (;;) {
    ...
    while (audio_pkt_size > 0) {
      ...
      if (got_frame) {
        ...
        data_size =
            audio_resampling(videoState, avFrame, AV_SAMPLE_FMT_S16, audio_buf);
      ...
      *pts_ptr = pts;
      ...
      videoState->audio_clock +=
          (double)data_size / (double)(n * videoState->audio_ctx->sample_rate);
    ...
    if (avPacket->pts != AV_NOPTS_VALUE) {
      videoState->audio_clock =
          av_q2d(videoState->audio_st->time_base) * avPacket->pts;
    }
  }
 ...
 * @param   out_sample_fmt      audio output sample format (e.g.
 * AV_SAMPLE_FMT_S16).
...
static int audio_resampling(VideoState *videoState,
  ...
  AudioResamplingState *arState =
      getAudioResampling(videoState->audio_ctx->channel_layout);
  ...
  av_opt_set_int(arState->swr_ctx, "in_channel_layout",
                 arState->in_channel_layout, 0);

  // Set SwrContext parameters for resampling
  av_opt_set_int(arState->swr_ctx, "in_sample_rate",
                 videoState->audio_ctx->sample_rate, 0);

  // Set SwrContext parameters for resampling
  av_opt_set_sample_fmt(arState->swr_ctx, "in_sample_fmt",
                        videoState->audio_ctx->sample_fmt, 0);

  // Set SwrContext parameters for resampling
  av_opt_set_int(arState->swr_ctx, "out_channel_layout",
                 arState->out_channel_layout, 0);

  // Set SwrContext parameters for resampling
  av_opt_set_int(arState->swr_ctx, "out_sample_rate",
                 videoState->audio_ctx->sample_rate, 0);
  ...
  arState->max_out_nb_samples = arState->out_nb_samples =
      av_rescale_rnd(arState->in_nb_samples, videoState->audio_ctx->sample_rate,
                     videoState->audio_ctx->sample_rate, AV_ROUND_UP);
  ...
  arState->out_nb_channels =
      av_get_channel_layout_nb_channels(arState->out_channel_layout);

  // allocate data pointers array for arState->resampled_data and fill data
  // pointers and linesize accordingly
  ret = av_samples_alloc_array_and_samples(
      &arState->resampled_data, &arState->out_linesize,
      arState->out_nb_channels, arState->out_nb_samples, out_sample_fmt, 0);
  ...
  arState->out_nb_samples = av_rescale_rnd(
      swr_get_delay(arState->swr_ctx, videoState->audio_ctx->sample_rate) +
          arState->in_nb_samples,
      videoState->audio_ctx->sample_rate, videoState->audio_ctx->sample_rate,
      AV_ROUND_UP);
  ...
  if (arState->out_nb_samples > arState->max_out_nb_samples) {
    ...
    ret = av_samples_alloc(arState->resampled_data, &arState->out_linesize,
                           arState->out_nb_channels, arState->out_nb_samples,
                           out_sample_fmt, 1);
  ...
  if (arState->swr_ctx) {
    ...
    ret = swr_convert(arState->swr_ctx, arState->resampled_data,
    ...
    arState->resampled_data_size = av_samples_get_buffer_size(
        &arState->out_linesize, arState->out_nb_channels, ret, out_sample_fmt,
        1);
 ...
 */
AudioResamplingState *getAudioResampling(uint64_t channel_layout) {
  AudioResamplingState *audioResampling =
      av_mallocz(sizeof(AudioResamplingState));
